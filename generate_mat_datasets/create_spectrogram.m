function [Data_spec_MTI2, MD] = create_spectrogram(filename)
% 
% % RETURNS: Spectrogram and STFT parameters
% 
% % EXTRACT SOME BASIC INFO FROM THE .DAT
...
%     Tsweep = radarData(2)/1000; % Sweep time in seconds
% 
%     % The data is 1D but every 128 or NTS samples we get a new
%     % slow time(pulse with listening in pulsed radar). THIS IS FAST TIME
%     % with which we get the 
%     NTS = radarData(3); % Number of time samples per beat note/chirp
% 
...
% 
%     % THIS IS SLOW TIME(each pulse in pulse radar but now its each chirp)
%     nc=record_length/Tsweep; % number of chirps
% 
% % CREATE RANGE-TIME MATRIX(code provided already)
...
% 
%     Data_spec_MTI2= 20*log10(abs(flipud(Data_spec_MTI2)));
% 
% 
% 
% 
